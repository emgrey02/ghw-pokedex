...
export default function PaginationBar(props: { totalPages: number }) {
    ...
    const par = params.toString().split('&');
    ...
    useEffect(() => {
        if (Number(currentPage) <= 1) {
            ...
            return;
        } else if (Number(currentPage) >= props.totalPages) {
    ...
    const pages = Array.from(
        { length: +`${props.totalPages < 5 ? props.totalPages : 5}` },
        (_, i) => {
            let pageNum = Number(currentPage);
            if (pageNum <= 3) return i + 1;
            if (pageNum >= props.totalPages - 2)
                return i + props.totalPages - 4;
    ...
    return (
        <ul className='w-full md:w-[60vw] lg:w-[40vw] flex justify-between items-center h-min'>
            ...
            {pages.map((p) => (
                ...
                >
                    <Link
                        ...
                        href={`/?page=${p}${params.toString().includes('query') ? `&${par[1]}` : ''}`}
                    >
                        {p}
                    </Link>
                </li>
            ...
            <li>
                <div
                    className={`${+currentPage >= props.totalPages - 2 ? 'invisible' : ''}`}
                >
                    ...
                </div>
            </li>
            ...
            <li>
                <Link
                    href={`/?page=${props.totalPages}${params.toString().includes('query') ? `&${par[1]}` : ''}`}
                >
                    <svg
                        className='stroke-slate-700 hover:stroke-slate-900 bg-transparent hover:bg-indigo-900/60 rounded border-slate-700 disabled:border-slate-500 disabled:bg-inherit transition focus:outline-none focus:ring-2 ring-slate-800 hover:fill-slate-900  disabled:fill-gray-400'
                        xmlns='http://www.w3.org/2000/svg'
