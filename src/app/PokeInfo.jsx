            ...
            <div id='card' className={`place-self-center z-20 text-zinc-700 dark:text-gray-400 w-fit h-min flex flex-col gap-4 p-8 bg-zinc-400/50 dark:bg-zinc-600/50 outline outline-slate-600 shadow-xl shadow-slate-900/30 rounded-md`}>
                <div className='grid grid-cols-2 h-min w-full gap-6 pb-10'>
                    <div className='flex flex-col ring-1 ring-indigo-900 dark:ring-indigo-300 p-4 self-start'>
                        <button title='Hear My Cry!' className=' hover:bg-gray-500/30 rounded-full flex flex-col items-center justify-center transition-all' onClick={()=> (uid === "url" ? stop() : playAudio())}>
                            {currentPoke.sprites.front_default || currentPoke.sprites.front_shiny ? (
                                <Image
                                    className={playing ? 'animate-ping' : ''}
                                    src={currentPoke.sprites.front_default || currentPoke.sprites.front_shiny}
                                    width={150}
                                    height={150}
                ...
                <div className='grid grid-cols-3 items-center'>
                    ...
                    <div className='flex flex-col items-center justify-center'>
                        <p className='text-2xl md:text-3xl text-indigo-900 dark:text-indigo-300 font-semibold transition-all'>{currentPoke.height / 10 || 'N/A'} m</p>
                        <h3 className='text-xs md:text-sm transition-all'>Height</h3>
                    </div>
                    <div className='flex flex-col items-center justify-center'>
                        <p className='text-2xl md:text-3xl text-indigo-900 dark:text-indigo-300 font-semibold transition-all'>{currentPoke.base_experience || 'N/A'}</p>
                ...
                <div className='grid grid-cols-3 grid-row-auto gap-8'>
                    ...
                    (
                        <div className='flex flex-col items-center' key={index}>
                            <p className='text-2xl md:text-3xl text-indigo-900 dark:text-indigo-300 font-semibold transition-all'>{stat.base_stat || 'N/A'}</p>
                            <h3 className='text-xs md:text-sm text-center transition-all'>{stat.stat.name}</h3>
                        </div>
                    ))
                    }
